...
def generate_html(result: DietResult) -> str:
    ...
    # Блок «как отмерить ложками» нужен только при МЕЛКИХ порциях (мелкие собаки):
    # если даже самый крупный продукт суточной раскладки < 90 г. Большим собакам
    # (мясо сотнями грамм — 250 г ≈ 17 ложек) ложки бессмысленны, им просто весы.
    _max_portion = max((v for v in result.distribution.values() if v > 0), default=0)
    if _max_portion >= 90:
        measure_legend = ""
...
<section class="page ai-analysis">
  ...
  <p class="section-sub" style="font-size:10pt;">Анализ на основе ветеринарных стандартов NRC, FEDIAF и AAFCO. Мы учитываем породные особенности, кондицию, заболевания и баланс нутриентов — и объясняем, почему рацион составлен именно так. Отчёт проверен нашим специалистом.</p>
