...
ЗАПРЕЩЕНЫ ЛЮБЫЕ ЦИФРЫ: ни граммов, ни процентов, ни килограммов, ни количества
кормлений или продуктов цифрой. Все числа уже есть в таблицах ниже и
авторитетны только там. Если хочется назвать число — назови словами
(«утреннее и вечернее кормление», «целевой вес», «нежирное мясо»).

ХОРОШИЙ ПРИМЕР:
«Этот рацион рассчитан специально для Барона — с учётом его породы, веса и активности. Лабрадоры склонны к набору веса, поэтому мы сделали акцент на нежирных белках и контроле порций. Точные граммовки и список продуктов — в таблицах ниже; рацион разделён на утреннее и вечернее кормление.»

Пиши конкретно: какие особенности породы учтены, почему именно такой тип питания, что важного в расчёте — но без чисел.
{gender_note}
...
ЭТО — ЧЕК-ЛИСТ ДЕЙСТВИЙ владельца: как хранить, как вводить новые продукты, как
чередовать, на что смотреть, как обрабатывать. НЕ объясняй «почему рацион такой»
(это в анализе) и НЕ пересказывай таблицы. КАЖДЫЙ пункт — новое практическое
действие. ЗАПРЕЩЕНО повторять то, что и так очевидно из таблиц/анализа:
- «делите на N кормлений» (число кормлений уже в таблице),
- «добавляйте кальций / выравнивайте Ca:P» (доза в таблице добавок),
- «регулярно взвешивайте и сверяйтесь с целевым весом» — допустимо ОДНОЙ
  короткой строкой максимум, не несколькими.
Дай то, чего НЕТ в других разделах: хранение и заморозка, ввод новых продуктов,
ротация белков, признаки непереносимости, температура подачи, сезонные нюансы.

ЗАПРЕЩЕНЫ ЛЮБЫЕ ЦИФРЫ: ни граммов, ни процентов, ни килограммов, ни частоты
цифрой («2 раза в неделю»), ни сроков хранения числом. Где нужно количество —
словами: «небольшими порциями», «пару раз в неделю», «изредка», «регулярно».

ЗАПРЕЩЕНО (канцелярит): «уникальный», «оптимальный», «сбалансированный подход», «индивидуальный», «замечательный», «рекомендуется». Не лей воду.
...
ХОРОШИЙ ПРИМЕР (действия, без повторов и воды):
• Говядину и индейку чередуйте — не давайте одно и то же мясо несколько дней подряд. Это снижает риск непереносимости.
• Печень давайте умеренно и небольшими порциями — избыток витамина A токсичен.
• Новые продукты вводите по одному и маленькими порциями, наблюдая за стулом пару дней.
• Замораживайте порции в zip-пакетах, выдавив воздух — так мясо долго хранится без потери качества.

Пиши так же: конкретно, по делу, но без чисел. Упоминай кличку. Без emoji.""", max_tokens=800)
    return result or ""
...
2. Вес и кондиция — текущий vs целевой, почему так, и как отслеживать (рёбра/талия,
   взвешивание) — ВСЁ про вес в ОДНОМ этом блоке, отдельного блока про контроль веса не делай
...
9. Образ жизни — советы по активности + породе (без повтора темы веса)

Требования:
- {gender_note}
- Обращайся на «вы» (с маленькой буквы, НЕ «Вы»), упоминай кличку
- Каждый блок — 3-5 предложений с КОНКРЕТИКОЙ (продукты, особенности породы, логика выбора)
- КАЖДАЯ тема — РОВНО в одном блоке. НЕ повторяй один и тот же риск или совет в
  разных блоках (например, заворот желудка, контроль веса, добавку кальция, заморозку
  упоминай ОДИН раз — там, где это уместнее всего).
- «Вес и кондиция» и «Контроль веса» — это ОДНА тема, бери что-то одно (оценку
  кондиции по рёбрам/талии упоминай не более чем в одном блоке).
- Заморозку, хранение, температуру подачи, мытьё мисок и прочие ДЕЙСТВИЯ в анализ
  НЕ пиши — они идут в раздел заметок. Анализ объясняет ПОЧЕМУ, а не как делать.
- Блок «Продукты» — НЕ общий ликбез («мясо — это белок, овощи — клетчатка, варить
  без соли»). Пиши, почему ИМЕННО эти продукты для ЭТОЙ собаки: её порода, диагноз,
  кондиция, что заменили и чем. Если нечего сказать персонального — лучше выбери
  другую тему из списка.
- ЗАПРЕЩЕНЫ ЛЮБЫЕ ЦИФРЫ: ни граммов, ни процентов, ни килограммов, ни Ca:P числом, ни количества кормлений. Все числа из расчёта уже показаны в таблицах и авторитетны только там — НЕ цитируй и НЕ пересчитывай их в тексте. Где нужна величина — пиши словами («целевой вес», «нежирное мясо», «небольшая доля субпродуктов»).
