...
def generate_personal_analysis(dog_profile: dict, diet_summary: dict) -> str:
    ...
    result = _ask(f"""Ты — ветеринарный диетолог с 20-летним стажем. Напиши глубокий персональный анализ рациона для владельца собаки.
...
5. ЕСЛИ ЕСТЬ ДИАГНОЗЫ — отдельный блок (tag: health) на каждый значимый диагноз. При панкреатите: почему снижен жир, почему убраны жирные субпродукты, почему важно не давать лишнего. При гастрите: почему нет сырых костей, почему варёный рацион щадящий. При аллергии: какие белки исключены, почему ротация белков важна. И т.д.

6. ЕСЛИ ПРОБЛЕМНЫЙ СТУЛ (tag: health) — возможные причины (паразиты, дисбактериоз, непереносимость, стресс). Что в рационе поможет (пробиотик, тыква, клетчатка). Рекомендация сдать анализ кала.

7. ЕСЛИ ЩЕНОК (tag: lifestyle) — что сейчас критично (Ca:P, скорость роста, не перекормить). Когда пересчитать рацион. Чем питание щенка крупной породы отличается от мелкой.

8. ЕСЛИ БЕРЕМЕННОСТЬ/ЛАКТАЦИЯ (tag: health) — почему увеличена калорийность, на что обратить внимание.

9. ОБРАЗ ЖИЗНИ (tag: lifestyle) — конкретный совет на основе совокупности факторов (активность + кондиция + порода). Например: «при средней активности и склонности к полноте взвешивайте раз в 2 недели».

Требования:
- Обращайся на «вы», упоминай кличку
- Каждый блок — 2-4 предложения, но содержательные
- Анализируй СОВОКУПНОСТЬ факторов, не каждый по отдельности: «лабрадор + перевес + средняя активность = рацион с дефицитом калорий + хондропротекторы + контроль веса»
- Пиши как практикующий диетолог, а не как учебник
- Без emoji. Без воды. Каждое предложение несёт конкретную информацию.
- Пиши 5-8 блоков в зависимости от количества значимых факторов
- НЕ пиши блоки про факторы, которых у этой собаки нет (нет диагнозов — не пиши про диагнозы)
