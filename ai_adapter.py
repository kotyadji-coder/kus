...
def generate_natural_product_notes(dog_profile: dict, warnings: list) -> str:
    ...
    gender_note = _gender_note(sex)
    result = _ask(f"""Напиши 5-7 конкретных рекомендаций для владельца собаки по натуральному питанию.

{gender_note}
Профиль: {json.dumps(dog_profile, ensure_ascii=False)}
Предупреждения системы: {warnings}

Формат: маркированный список (начинай каждый пункт с «•»). Каждый пункт — 1-2 предложения.
ЗАПРЕЩЕНО: канцелярит, вода, «уникальный», «оптимальный», «индивидуальный подход». «вы» с маленькой буквы.
Пиши конкретно — цифры, названия продуктов, граммы, сроки. Без emoji.
Темы: почему эти продукты, на что смотреть при кормлении, когда пересчитать рацион, хранение.""", max_tokens=600)
    return result or ""
