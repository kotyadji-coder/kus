...
def generate_personal_analysis(dog_profile: dict, diet_summary: dict) -> str:
    ...
    result = _ask(f"""Ты — ветеринарный диетолог с 20-летним стажем. Напиши персональный анализ для владельца собаки.

Профиль собаки:
{json.dumps(dog_profile, ensure_ascii=False)}

Результаты расчёта:
{json.dumps(diet_summary, ensure_ascii=False)}

Напиши 5-6 блоков анализа. Каждый блок — про конкретный аспект здоровья и питания этой собаки.

Формат ответа — строго HTML, без обёртки, только содержимое блоков:

<div class="insight">
  <div class="tag breed">Порода</div>
  <p>Текст про особенности породы и питания...</p>
</div>

<div class="insight">
  <div class="tag health">Здоровье</div>
  <p>Текст про здоровье, вес, кондицию...</p>
</div>

Используй теги: breed (порода), health (здоровье), nutrition (питание), lifestyle (образ жизни).

Требования:
- Обращайся на «вы», упоминай кличку собаки
- Пиши конкретно про ЭТУ собаку, не общие фразы
- Объясни, почему рацион подобран именно так
- Упомяни риски породы (ожирение, аллергии, суставы)
- Дай 1-2 практических совета по образу жизни
- Если есть диагнозы — объясни, как рацион их учитывает
- Если щенок — про рост и когда менять рацион
- Если стул жидкий или проблемный — объясни возможные причины (паразиты, непереносимость, дисбактериоз, низкое качество корма) и порекомендуй обратиться к ветеринару для исключения серьёзных причин
- Если есть проблемы с кожей/шерстью — объясни связь с питанием
- Тон: спокойный, профессиональный. Без emoji. Без маркетинга.
- Каждый блок — 2-3 предложения, не больше.
- Общий объём: 800-1200 символов.
- В конце ОБЯЗАТЕЛЬНО добавь блок-дисклеймер:
