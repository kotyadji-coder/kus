...
PROFILES = [
    ...
    # --- Регрессионные профили (храповик: найдено фаззером 2026-06-18, чинить нельзя молча) ---
    {"name": "Малыш", "breed": "Лабрадор", "age_months": 2, "sex": "male", "neutered": False,
     "weight_kg": 8, "condition": "ideal", "activity": "puppy", "diet_type": "barf",
     "budget": "market", "stop_products": [], "diagnoses": [],
     "label": "Щенок <4 мес (4 кормления = 4 слота меню)"},

    {"name": "Тимка", "breed": "Йоркширский терьер", "age_months": 24, "sex": "male", "neutered": True,
     "weight_kg": 4, "condition": "ideal", "activity": "medium", "diet_type": "cooked",
     "budget": "supermarket", "stop_products": ["курица", "говядина", "индейка"], "diagnoses": [],
     "label": "Тройной стоп мяса (эскалация бюджета, не веге-перекос)"},
]
...
def run_evals() -> list[dict]:
    ...
    for profile in PROFILES:
        ...
        for day in result.weekly_menu:
            for p in day.morning + getattr(day, 'midday', []) + getattr(day, 'afternoon', []) + day.evening:
        ...
        day_totals = []
        for day in result.weekly_menu:
            total_g = (sum(p.grams for p in day.morning) + sum(p.grams for p in getattr(day, 'midday', []))
                       + sum(p.grams for p in getattr(day, 'afternoon', [])) + sum(p.grams for p in day.evening))
        ...
        total += 1
        total_products = sum(len(d.morning) + len(getattr(d, 'midday', [])) + len(getattr(d, 'afternoon', [])) + len(d.evening) for d in result.weekly_menu)
        ...
        # Единый источник — reconcile.py (его же гоняют фаззер/снапшоты/канарейка).
        # ============================================================
        html = None
        try:
            import pdf_generator
            html = pdf_generator.generate_html(result)  # рендер без AI; добавляет заметку щенка в warnings
        except Exception:
            html = None

        for c in reconciliation_checks(result, html):
            total += 1
            checks.append((c["key"], c["status"], c["detail"]))
            passed += 1 if c["status"] == "ok" else 0.5 if c["status"] == "warn" else 0
